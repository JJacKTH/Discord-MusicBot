...
module.exports = {
  name: "lyrics",
  description: "แสดงเนื้อเพลงของเพลงที่ค้นหา",
  usage: "[Song Name]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] && !player) return client.sendTime(message.channel, "❌ | **ไม่มีเพลงเล่นในขณะนี้...**");
    if (!args[0]) SongTitle = player.queue.current.title;

    let lyrics = await lyricsFinder(SongTitle);
    if (!lyrics) return client.sendTime(message.channel, `**ไม่พบเนื้อเพลงสำหรับ -** \`${SongTitle}\``);
    lyrics = lyrics.split("\n"); //spliting into lines
  ...
  SlashCommand: {
    options: [
      {
        ...
        value: "song",
        type: 3,
        description: "ป้อนชื่อเพลง เพื่อค้นหา",
        required: false,
      },
    ],
    /**
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!interaction.data.options && !player) return client.sendTime(interaction, "❌ | **ไม่มีเพลงเล่นในขณะนี้...**");
      ...
      if (lyrics.length === 0)
        return client.sendTime(interaction, `**ไม่พบเนื้อเพลงสำหรับ -** \`${SongTitle}\``);
      lyrics = lyrics.split("\n"); //spliting into lines
      let SplitedLyrics = _.chunk(lyrics, 40); //45 lines each page

      let Pages = SplitedLyrics.map((ly) => {
        let em = new MessageEmbed()
          .setAuthor(`เนื้อเพลง: ${SongTitle}`, client.config.IconURL)
          .setColor("RANDOM")
          .setDescription(ly.join("\n"));
