...
module.exports = {
  name: "play",
  description: "เล่นเพลงโปรดของคุณ",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x:|**คุณต้องอยู่ช่องพูดคุยเพื่อใช้งาน Bot!**");
    let SearchString = args.join(" ");
    if (!SearchString) return client.sendTime(message.channel, `**กรุณากรอก - **\`${GuildDB.prefix}เพื่อ เล่นเพลง\``);
    let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
    let Searching = await message.channel.send(":mag_right: หาเพลง...");
    ...
    if (!player) return client.sendTime(message.channel, "❌ | **ไม่มีลำดับเพลงที่กำลังเล่นอยู่...**");
    ...
    try {
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          SongAddedEmbed.setAuthor(`เพิ่มเพลย์ลิสต์ลงในคิว`, message.author.displayAvatarURL());
          SongAddedEmbed.addField("เข้าคิว", `\`${Searched.tracks.length}\` songs`, false);
          ...
          SongAddedEmbed.setAuthor(`เพิ่มแนวเพลง`, client.config.IconURL);
          SongAddedEmbed.setDescription(`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri})`);
          SongAddedEmbed.addField("ผู้เขียน", Searched.tracks[0].info.author, true);
          //SongAddedEmbed.addField("Duration", `\`${prettyMilliseconds(Searched.tracks[0].length, { colonNotation: true })}\``, true);
          if (player.queue.totalSize > 1) SongAddedEmbed.addField("เพิ่มแนวเพลง", `${player.queue.size - 0}`, true);
          Searching.edit(SongAddedEmbed);
        } else {
          return client.sendTime(message.channel, "**ไม่พบเพลง - **" + SearchString);
        }
      } else {
        let Searched = await player.search(SearchString, message.author);-
        if (!player) return client.sendTime(message.channel, "❌ | **ไม่มีลำดับเพลงที่กำลังเล่นอยู่...**");

        if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**ไม่พบเพลง - **" + SearchString);
        else if (Searched.loadType == "PLAYLIST_LOADED") {
          player.queue.add(Searched.tracks);
          if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
          SongAddedEmbed.setAuthor(`เพิ่มเพลย์ลิสต์ลงในคิว`, client.config.IconURL);
          SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
          SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
          SongAddedEmbed.addField("เข้าคิว", `\`${Searched.tracks.length}\` songs`, false);
          SongAddedEmbed.addField("ระยะเวลาของเพลย์ลิสต์", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
          Searching.edit(SongAddedEmbed);
        } else {
          player.queue.add(Searched.tracks[0]);
          if (!player.playing && !player.paused && !player.queue.size) player.play();
          SongAddedEmbed.setAuthor(`เพิ่มคิว`, client.config.IconURL);

          SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
          SongAddedEmbed.setDescription(`[${Searched.tracks[0].title}](${Searched.tracks[0].uri})`);
          SongAddedEmbed.addField("ผู้เขียน", Searched.tracks[0].author, true);
          SongAddedEmbed.addField("ความยาว", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "เล่นเพลงในห้องที่คุณอยู่",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!member.voice.channel) return client.sendTime(interaction, "❌ | **คุณต้องอยู่ในห้องเพิ่อใช้คำสั่ง!**");
      if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, ":x: | *คุณต้องอยู่ในห้องเพิ่อใช้คำสั่ง!**");
      let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
      if (!CheckNode || !CheckNode.connected) {
